/*
   ...
   2  GRIGIO ----> POT COMUNE
   3  GIALLO ----> POT -
   4  VERDE -----> AVVOLGIMENTO CCW
*/
uint8_t L_T = 0;                           //locazione di memoria per memorizzazione ULTIMO TARGET AZIMUT RICHIESTO
uint8_t L_A = 1;                           //locazione di memoria per memorizzazione ULTIMA POSIZIONE AZIMUT CORRENTE CONOSCIUTA
uint8_t L_E = 2;                           //locazione di memoria per memorizzazione ULTIMA POSIZIONE ELEVAZIONE CONOSCIUTA
uint8_t POT_MIN = 3 ;                      //locazione di memoria per memorizzazione valore pot min
uint8_t POT_MAX = 4 ;                      //locazione di memoria per memorizzazione valore pot max
...
/*-----Indice delle pagine-----*/
const byte PAG_SPLASH = 0;              // Pagina di caricamento
const byte PAG_MAIN = 1;                // Pagina principale con le info
const byte PAG_MANUAL = 2;              // Pagina per inserimento manuale e richiamo memorie
const byte PAG_CONFIG = 3;              // Pagina per attivare/disattivare funzionalità (trovare il modo per memorizzarle permanentemente)
...
uint32_t  VAR_AZIMUT_LAST_TARGET = ((EEPROM.read(0) * 4));  // VAriabile per memorizzare ultimo target richiesto.
...
/*-----colori Nextion-----*/
const unsigned int NORMAL = 62819;
const unsigned int OVERLAP = 63488;
const unsigned int SFONDO = 8484;
/*------------------------*/

/*--------timer--------*/
const unsigned short DELAY_S = 20;
const unsigned short DELAY_M = 50;
const unsigned short DELAY_L = 100;
unsigned short t_skew = 5;
unsigned short t_max = (AZIMUT_TARGET + 5);
unsigned short t_min = (AZIMUT_TARGET - 5);
...
/*
   GAUGE
*/
NexGauge GAU_AZIMUT = NexGauge(PAG_MAIN, 9, "GAU_AZIMUT");   //dichiarazione oggetto gauge azimut
NexGauge GAU_ELEVAZ = NexGauge(PAG_MAIN, 10, "GAU_ELEVAZ");    //dichiarazione oggetto gauge elevazione
